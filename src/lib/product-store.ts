...
import { resolveProductCategories } from "@/lib/products";
...
export const PRODUCT_CATEGORIES = ASSIGNABLE_CATEGORIES.map(
  (category) => category.slug,
) as [string, ...string[]];

export function validateProductInput(
  input: unknown,
  categories: Category[] = ASSIGNABLE_CATEGORIES,
) {
  if (!input || typeof input !== "object")
    return { ok: false as const, error: "Payload không hợp lệ" };
  ...
  const ratingCount =
    value.ratingCount === undefined ? 0 : Number(value.ratingCount);
  const active = value.active !== false;
  const categorySlugs = categories.map((item) => item.slug);
  const selectedCategories = Array.from(
    new Set(
      (Array.isArray(value.categories) ? value.categories : [category]).filter(
        (item): item is string =>
          typeof item === "string" && categorySlugs.includes(item),
      ),
    ),
  );
  ...
  if (!/^[a-z0-9]+(?:-[a-z0-9]+)*$/.test(slug))
    return {
      ok: false as const,
      error: "Slug chỉ được gồm chữ thường, số và dấu gạch ngang",
    };
  if (!name || name.length > 180)
    return {
      ok: false as const,
      error: "Tên sản phẩm bắt buộc và tối đa 180 ký tự",
    };
  if (!selectedCategories.length)
    return {
      ok: false as const,
      error: "Hãy chọn ít nhất một danh mục hợp lệ",
    };
  if (!Number.isFinite(price) || price <= 0)
    return { ok: false as const, error: "Giá phải lớn hơn 0" };
  if (
    originalPrice !== undefined &&
    (!Number.isFinite(originalPrice) || originalPrice <= 0)
  )
    return { ok: false as const, error: "Giá gốc không hợp lệ" };
  ...
  if (!Number.isFinite(rating) || rating < 0 || rating > 5)
    return { ok: false as const, error: "Rating phải từ 0 đến 5" };
  if (!Number.isInteger(ratingCount) || ratingCount < 0)
    return {
      ok: false as const,
      error: "Số đánh giá phải là số nguyên không âm",
    };
  ...
  const product: ProductRecord = {
    ...
    ...(originalPrice !== undefined && originalPrice > price
      ? { badge: `-${Math.round((1 - price / originalPrice) * 100)}%` }
      : {}),
...
export function validateCategoryInput(input: unknown) {
  ...
  const name = typeof value.name === "string" ? value.name.trim() : "";
  const shortName =
    typeof value.shortName === "string" ? value.shortName.trim() : name;
  const color =
    typeof value.color === "string" ? value.color.trim() : "#13233F";
  const icon =
    typeof value.icon === "string"
      ? value.icon.trim().toLowerCase()
      : "sparkle";
  const active = value.active !== false;

  if (!/^[a-z0-9]+(?:-[a-z0-9]+)*$/.test(slug))
    return { ok: false as const, error: "Slug danh mục không hợp lệ" };
  if (!name || name.length > 80)
    return {
      ok: false as const,
      error: "Tên danh mục bắt buộc và tối đa 80 ký tự",
    };
  if (!shortName || shortName.length > 40)
    return {
      ok: false as const,
      error: "Tên ngắn bắt buộc và tối đa 40 ký tự",
    };
  if (!/^#[0-9a-f]{6}$/i.test(color))
    return { ok: false as const, error: "Màu phải ở dạng HEX, ví dụ #B83B68" };
  if (!/^[a-z0-9-]{1,24}$/.test(icon))
    return {
      ok: false as const,
      error: "Icon chỉ được gồm chữ thường, số và dấu gạch ngang",
    };
...
async function readGitHubJson<T>(
  ...
  if (!payload.content || !payload.sha)
    throw new Error("GitHub response thiếu content hoặc sha");
...
async function writeGitHubFile(
  ...
  if (!response.ok) {
    const detail = (await response
      .json()
      .catch(() => ({}))) as GitHubContentResponse;
    throw new Error(
      detail.message || `GitHub write failed (${response.status})`,
    );
  }
}
...
export async function readProducts(): Promise<ProductRecord[]> {
  ...
  const normalize = (products: ProductRecord[]) =>
    products.map((product) => {
      const productCategories = resolveProductCategories(product);
      return {
        ...product,
        category: productCategories[0] ?? "cham-soc-suc-khoe",
  ...
  try {
    return normalize(
      (await readGitHubJson<ProductRecord[]>(config, GITHUB_PRODUCTS_PATH))
        .data,
    );
  } catch (error) {
    console.error(
      "[Admin storage] GitHub read products failed; using bundled catalog:",
      error,
    );
    return normalize(await readLocalJson<ProductRecord[]>(DATA_PATH));
  }
...
export async function writeProducts(products: ProductRecord[]) {
  ...
  const current = await readGitHubJson<ProductRecord[]>(
    config,
    GITHUB_PRODUCTS_PATH,
  );
  await writeGitHubFile(
    config,
    GITHUB_PRODUCTS_PATH,
    content,
    "chore(admin): update products",
    current.sha,
  );
...
export async function readCategories() {
  ...
  try {
    return (await readGitHubJson<Category[]>(config, GITHUB_CATEGORIES_PATH))
      .data;
  } catch (error) {
    console.error(
      "[Admin storage] GitHub read categories failed; using bundled categories:",
      error,
    );
...
export async function writeCategories(categories: Category[]) {
  ...
  const current = await readGitHubJson<Category[]>(
    config,
...
function safeImagePath(slug: string, filename: string) {
  ...
  return {
    relativePath: `products/${slug}/${normalizedName}-${suffix}${extension}`,
    extension,
    allowed: allowedExtensions.has(extension),
  };
}
...
export async function uploadProductImage(slug: string, file: File) {
  ...
  if (file.size <= 0 || file.size > maxBytes)
    throw new Error("Ảnh phải lớn hơn 0 và không vượt quá 5 MB");
  const imagePath = safeImagePath(slug, file.name);
  if (!imagePath.allowed)
    throw new Error("Định dạng ảnh hỗ trợ: JPG, PNG, WebP, GIF hoặc AVIF");
  ...
  await writeGitHubFile(
    config,
    `public/${imagePath.relativePath}`,
    buffer,
    `chore(admin): upload product image for ${slug}`,
  );
