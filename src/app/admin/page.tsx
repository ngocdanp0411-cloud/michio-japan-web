...
export default function AdminPage() {
  ...
  const load = async () => {
    const res = await fetch(`/api/admin/products?q=${encodeURIComponent(q)}&cat=${encodeURIComponent(cat)}`);
    if (res.ok) {
      const data = await res.json();
  ...
  const save = async () => {
    ...
    const data = await res.json();
    if (res.ok) {
      setMsg(isNew ? "Đã thêm sản phẩm" : "Đã lưu");
      setEditing(null);
      setIsNew(false);
      load();
    ...
    return (
      <div className="min-h-[70vh] flex items-center justify-center p-4">
        <div className="w-full max-w-sm rounded-xl border bg-white p-6 shadow">
          ...
          <p className="text-sm text-gray-500 mt-1">Nhập mật khẩu để chỉnh giá, content, thêm sản phẩm</p>
          <input
            type="password"
            placeholder="Mật khẩu (mặc định: michio2024)"
            value={password}
  ...
  return (
    <div className="mx-auto max-w-[1280px] px-4 py-6">
      ...
      <div className="mt-4 flex flex-wrap gap-2">
        ...
        <button onClick={load} className="rounded-full border bg-white px-4 py-2.5 text-sm">
          Tải lại
        </button>
      </div>
          ...
          <div key={p.slug} className="flex gap-3 rounded-xl border bg-white p-3">
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="text-xs text-gray-500 line-clamp-1">
                {p.slug} • {p.category} • {new Intl.NumberFormat("vi-VN").format(p.price)}₫
              </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="w-full max-w-2xl max-h-[90vh] overflow-auto rounded-xl bg-white p-5">
            ...
            <div className="mt-4 flex gap-2 justify-end">
              ...
              <button onClick={save} className="rounded-full bg-[var(--michio-deep-rose)] px-5 py-2 text-sm font-semibold text-white">
                Lưu
              </button>
            </div>
            <div className="mt-3 text-xs text-gray-400">Lưu xong cần rebuild để Vercel thấy (sẽ tự deploy khi push Git). Local thì F5 là thấy ngay.</div>
          </div>
        </div>
      )}
    </div>
