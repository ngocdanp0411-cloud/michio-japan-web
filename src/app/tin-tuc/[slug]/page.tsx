  ...
  return (
    <div>
      <main className="mx-auto max-w-[1280px] px-4 py-7 md:py-10">
        ...
        <header className="mx-auto mt-7 max-w-4xl text-center md:mt-10"><Link href={`/tin-tuc/chuyen-muc/${post.category}`} className="michio-eyebrow hover:underline">{category?.name ?? "Michio Journal"}</Link><h1 className="michio-display mt-3 text-balance text-[2rem] uppercase md:text-6xl">{post.title}</h1><div className="mt-4 flex flex-wrap items-center justify-center gap-3 text-xs text-[var(--michio-text-subtle)]"><span>{new Intl.DateTimeFormat("vi-VN").format(new Date(post.publishedAt))}</span><span aria-hidden="true">•</span><span>Tác giả: {post.author}</span></div><p className="michio-body mx-auto mt-5 max-w-2xl text-base md:text-lg">{post.description}</p>{post.aiAssisted && <p className="mx-auto mt-3 max-w-2xl text-xs leading-5 text-[var(--michio-text-subtle)]">Bài viết được AI hỗ trợ tổng hợp từ các nguồn được dẫn và chỉ nhằm mục đích cung cấp thông tin.</p>}</header>
