...
export function ProductMenu({
  ...
  const [menuPosition, setMenuPosition] = useState({ top: 0, left: 16, width: 320, maxHeight: 480 });
  ...
  const updatePosition = useCallback(() => {
    ...
    const inset = 16;
    const bottomClearance = window.innerWidth < 768 ? 76 : 12;
    const width = Math.min(mobile ? 358 : 340, window.innerWidth - inset * 2);
    setMenuPosition({
      top: rect.bottom,
  ...
  return (
    <div className="group relative shrink-0">
      <button
        ...
        className={`flex h-11 min-w-11 items-center justify-center gap-2 rounded-full px-2 text-left text-xs font-semibold transition-colors duration-200 hover:bg-[var(--michio-surface-muted)] hover:text-[var(--michio-primary)] motion-reduce:transition-none lg:min-h-12 lg:min-w-[190px] lg:justify-start lg:gap-3 lg:rounded-none lg:px-4 lg:text-sm ${open ? "bg-[var(--michio-primary-soft)] text-[var(--michio-primary)]" : "text-[var(--michio-text)]"}`}
      >
        ...
        <span className="sr-only lg:not-sr-only lg:min-w-[128px] lg:whitespace-nowrap">{open ? "Đóng danh mục" : "Danh mục sản phẩm"}</span>
        <svg
          width="18"
          height="18"
          viewBox="0 0 24 24"
      ...
      <div
        ...
        inert={!open}
        style={{ ...menuPosition }}
        className={`fixed z-[70] origin-top overflow-y-auto overscroll-contain rounded-2xl border border-[var(--michio-border)] bg-white p-2 text-[var(--michio-text)] shadow-[0_18px_48px_rgba(20,20,24,0.14)] transition-[transform,opacity] duration-200 motion-reduce:transition-none ${open ? "pointer-events-auto translate-y-2 opacity-100" : "pointer-events-none -translate-y-1 opacity-0"}`}
      >
        <div role="group" aria-label="Điều hướng website" className="grid grid-cols-2 border-b border-[var(--michio-border)] pb-2 lg:hidden">
          ...
          {navigationItems.map((item) => {
            const className = "flex min-h-11 items-center justify-between rounded-lg px-3 py-2 text-sm font-medium hover:bg-[var(--michio-surface-muted)] hover:text-[var(--michio-primary)]";
        ...
        <Link
          ...
          className="col-span-2 flex min-h-11 items-center justify-between rounded-lg bg-[var(--michio-primary-soft)] px-3 py-2 text-sm font-semibold text-[var(--michio-primary)] lg:col-span-1"
        >
          Tất cả sản phẩm
        ...
        {categories.map((category) => (
          <Link
            ...
            onClick={() => setOpen(false)}
            className="flex min-h-11 items-center justify-between rounded-lg px-3 py-2 text-sm font-medium hover:bg-[var(--michio-surface-muted)] hover:text-[var(--michio-primary)]"
          >
            {category.name}
