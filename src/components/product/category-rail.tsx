  ...
  return (
    <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-6 md:gap-4">
      {CATEGORIES.map((category) => (
        ...
        >
          <span className="font-display text-base font-extrabold leading-tight text-[var(--michio-text)] transition-colors duration-200 group-hover:text-[var(--michio-primary)] md:text-lg">
            {category.name}
          </span>
        </Link>
      ))}
