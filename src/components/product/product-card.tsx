...
export function ProductCard({ p }: { p: Product }) {
  ...
  return (
    <article className="group flex min-w-0 flex-col bg-white">
      <Link href={`/san-pham/${p.slug}`} prefetch={false} className="relative aspect-square overflow-hidden rounded-xl border border-[var(--michio-border)] bg-white">
      ...
      <div className="flex flex-1 flex-col pb-2 pt-3">
        <p aria-label={`Đánh giá ${p.rating.toFixed(1)} trên 5, ${p.ratingCount} lượt đánh giá`} className="mb-2 text-xs text-[var(--michio-text-muted)]"><span aria-hidden="true" className="text-[var(--michio-primary)]">★</span> {p.rating.toFixed(1)} <span className="text-[var(--michio-text-subtle)]">({p.ratingCount})</span></p>
        ...
        <a href={LINKS.zalo} target="_blank" rel="noopener noreferrer" className="mt-auto inline-flex min-h-11 items-center justify-between gap-2 border-t border-[var(--michio-border)] text-xs font-medium hover:text-[var(--michio-primary)]">
          Tư vấn sản phẩm <span aria-hidden="true">↗</span>
        </a>
      </div>
    </article>
  );
...
export function ProductGrid({ products, columns = "home" }: { products: Product[]; columns?: "home" | "category" }) {
  const gridClass = columns === "category"
    ? "grid-cols-2 gap-x-4 gap-y-7 md:grid-cols-3 lg:grid-cols-4 lg:gap-x-6"
    : "grid-cols-2 gap-x-4 gap-y-7 md:grid-cols-3 lg:grid-cols-4 lg:gap-x-6";
  return (
